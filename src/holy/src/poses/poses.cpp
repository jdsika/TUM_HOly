...
void Poses::update() {
    ...
    lift_right =
            RoboPose( std::vector<LimbPose> {
                            LimbPose (Core::Limb::RIGHT_FOOT,  d2r(0), d2r(0), d2r(turning_angle), 0.02, 0,step_height), // Compensate x due to weak motors
                        }, "lift_right");

    lift_left =
            RoboPose( std::vector<LimbPose> {
                            LimbPose (Core::Limb::LEFT_FOOT,  d2r(0), d2r(0), d2r(turning_angle), -0.02, 0,step_height),
                        }, "lift_left");
    ...
    // Absolute poses:
    // Init
    init_shift_toleft = pose_default+shift_toleft;
    init_shift_toright = pose_default+shift_toright;
    init_lift_left = init_shift_toright+lift_left;
    init_lift_right = init_shift_toleft+lift_right;
    init_fwd_left = init_lift_left+fwd_left;
    init_fwd_right = init_lift_right+fwd_right;
    init_dual_right = init_fwd_right+ dual_right- shift_toleft - lift_right;
    init_shift_frontright = init_dual_right+ shift_toright + dual_right ;

    // Loop
    loop_lift_left = init_shift_frontright+lift_left;
    loop_fwd_left = loop_lift_left+fwd_left+fwd_left;
    loop_dual_left = loop_fwd_left+ dual_left- shift_toright - lift_left;
    loop_shift_frontleft = loop_dual_left+ shift_toleft + dual_left ;
    loop_lift_right = loop_shift_frontleft+lift_right;
    loop_fwd_right = loop_lift_right+fwd_right+fwd_right;
    loop_dual_right = loop_fwd_right+ dual_right- shift_toleft - lift_right;
    loop_shift_frontright = loop_dual_right + shift_toright + dual_right;

    // Stop
    stop_lift_left = init_shift_frontright+lift_left;
